...
最慢的排序算法之一，但是容易实现，假如将一组数字按顺序排列，较大的值会浮动到数组的右侧，较小的值则会浮动到数组的左侧。是因为算法会多次在数组中移动，比较相邻的数据，当左侧值大于右侧值时将他们进行互换。
...
插入排序有两个循环，外循环将数组元素挨个移动，而内循环则对外循环选中的元素以及它后面的那个元素进行比较，如果外循环选中的元素比内循环选中的元素小，那么数组元素会向右移动，为内循环中的这个元素腾出位置。
